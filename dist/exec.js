...
function init_default(context, config) {
  return __async(this, null, function* () {
    if (config.initCmd != null) {
      const exitCode = yield exec.exec(config.initCmd);
...
function publish_default(context, config) {
  return __async(this, null, function* () {
    if (config.publishCmd != null) {
      const exitCode = yield exec3.exec(config.publishCmd);
      context.logger.debug(`Process finished with exit code ${exitCode}`);
    }
  });
}
...
function success_default(context, config) {
  return __async(this, null, function* () {
    if (config.successCmd != null) {
      const exitCode = yield exec5.exec(config.successCmd);
      context.logger.debug(`Process finished with exit code ${exitCode}`);
    }
  });
}
...
function version_default(context, config) {
  return __async(this, null, function* () {
    if (config.versionCmd != null) {
      const exitCode = yield exec7.exec(config.versionCmd);
      context.logger.debug(`Process finished with exit code ${exitCode}`);
    }
  });
}
