...
async function ovsxInfo(extensionName) {
  try {
    const cmdOutput = await getExecOutput(await npxCmd("ovsx"), ["ovsx", "get", extensionName, "--metadata"]);
    return JSON.parse(cmdOutput.stdout);
  } catch {
  }
}
async function ovsxPublish(context, vsixPath) {
  const cmdArgs = ["ovsx", "publish"];
  if (vsixPath != null) {
    cmdArgs.push("--packagePath", vsixPath);
  } else if (fs2.existsSync(path4.join(context.rootDir, "yarn.lock"))) {
    cmdArgs.push("--yarn");
  }
...
async function vsceInfo(extensionName) {
  try {
    const cmdOutput = await getExecOutput(await npxCmd("vsce"), ["vsce", "show", extensionName, "--json"]);
    return JSON.parse(cmdOutput.stdout);
  } catch {
  }
}
async function vscePackage(context) {
  ...
  if (fs2.existsSync(path4.join(context.rootDir, "yarn.lock"))) {
    cmdArgs.push("--yarn");
  }
  if (context.version.prerelease != null) {
    cmdArgs.push("--pre-release");
  ...
  const cmdOutput = await getExecOutput(await npxCmd("vsce"), cmdArgs);
...
async function verifyToken(tool, publisher) {
  await exec(await npxCmd(tool), [tool, "verify-pat", publisher]);
}
...
async function init_default(context, config) {
  ...
  if (config.vscePublish !== false) {
    await verifyToken("vsce", packageJson.publisher);
  }
  if (config.ovsxPublish) {
    await verifyToken("ovsx", packageJson.publisher);
  }
}
...
async function publish_default(context, config) {
  ...
  if (config.vscePublish !== false) {
    const vsceMetadata = await vsceInfo(extensionName) || {};
    if (context.version.prerelease != null) {
      context.logger.warn("Cannot publish version with prerelease tag to VS Code Marketplace");
    } else if (!vsceMetadata.versions?.find((obj) => obj.version === packageJson.version)) {
      await vscePublish(context, vsixPath);
  ...
  if (config.ovsxPublish) {
    const ovsxMetadata = await ovsxInfo(extensionName) || {};
    if (!Object.keys(ovsxMetadata.allVersions || {}).includes(packageJson.version)) {
      let success = true;
      try {
        await ovsxPublish(context, vsixPath);
      } catch (err) {
