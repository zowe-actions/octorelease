...
```javascript
import { utils } from "@octorelease/npm";
...
Octorelease will import the plugin via Node and look through the exported object for methods named like the steps listed above. For example, if your plugin only had an `init` and `success` step, the `main` file for your object would need to `export` an object with `init` and `success` functions.
